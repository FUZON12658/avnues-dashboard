...
//           }`}
//         >
//           {navigationSections &&  navigationSections.map((section:any) => (
...
const Sidebar = () => {
  ...
  // Use useEffect to set the initial data (later you can replace this with API call)
  useEffect(() => {
    // This is where you'll make your API call later
    // For now, setting the static data
    // const staticNavigationData: NavigationSection[] = [
    //   {
    //     title: "Navigation",
    //     items: [
    //       {
    ...
    // Later, replace the above with your API call like this:
    // fetchNavigationData().then(data => setNavigationSections(data));
  }, [data]);
    ...
    if (item.type === "link") {
      return (
        <li key={item.label} onClick={() => setActiveItem(item.label)}>
          ...
          >
            <span className="mr-3 -mt-1">
              <HugeiconsIcon icon={getIconObject(item.icon)} />
            </span>
            <span
              className={`transition-all ease-in-out ${
                isCollapsed
